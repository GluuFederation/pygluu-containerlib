...
""".strip()
    ...
    expected = f"""
bindDN: {fake_manager.config.get("ldap_binddn").decode()}
bindPassword: {fake_manager.secret.get("encoded_ox_ldap_pw").decode()}
servers: {host}:{port}
ssl.trustStoreFile: {fake_manager.config.get("ldapTrustStoreFn").decode()}
ssl.trustStorePin: {fake_manager.secret.get("encoded_ldapTrustStorePass").decode()}
""".strip()
...
def test_sync_ldap_truststore(tmpdir, fake_manager):
    ...
    assert dest.read() == fake_manager.secret.get("ldap_pkcs12_base64").decode()
