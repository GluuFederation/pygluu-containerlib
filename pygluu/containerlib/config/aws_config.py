...
class AwsConfig(BaseConfig):
    ...
    - `GLUU_AWS_SECRETS_ENDPOINT_URL`: The URL of AWS secretsmanager service (if omitted, will use the one in specified region).
    - `GLUU_AWS_SECRETS_PREFIX`: The prefix name of the secrets (default to `gluu`).
    - `GLUU_AWS_SECRETS_REPLICA_FILE`: The location of file contains replica regions definition (if any). This file is mostly used in primary region.
    ...
    [gluu]
    aws_access_key_id = GLUU_ACCESS_KEY_ID
    aws_secret_access_key = GLUU_SECRET_ACCESS_KEY
    ```
    ...
    def client(self) -> boto3.session.Session.client:
        ...
        try:
            client = boto3.client(
                "secretsmanager",
                endpoint_url=os.environ.get("GLUU_AWS_SECRETS_ENDPOINT_URL") or None,
            )
            return client
    ...
    def _prepare_secret(self) -> None:
        ...
        try:
            ...
            create_secret = partial(
                ...
                Name=self.basepath,
                SecretString=_dump_value({}),
                Description="Non-sensitive secrets (configs) for Gluu cluster",
            )
    ...
    def replica_regions(self) -> list[dict[str, _t.Any]]:
        ...
        The location of the file is pointed by `GLUU_AWS_SECRETS_REPLICA_FILE` environment variable.
        """
        regions = []

        with suppress(FileNotFoundError, TypeError, IsADirectoryError):
            file_ = os.environ.get("GLUU_AWS_SECRETS_REPLICA_FILE", "")
            try:
